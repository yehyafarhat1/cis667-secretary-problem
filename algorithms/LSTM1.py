...
class Net20211217(tr.nn.Module):
    def __init__(self):
        super(Net20211217, self).__init__()

    def set(self, dictionary, candidate_score, hidden_size):
    ...
    def predict_list(self,
        ...
        x = self.dictionary[candidate_list[0]]
        # y, v = net(x, v)
        y, v = self(x, v)
        y = y.squeeze()
        w = y.argmax()
        score = self.candidate_score[w]
        prob = y[w]
        i = 0
    ...
    def make_decision(self, candidate_lst):
        lstm_prediction_lst_length = len(self.predict_list(
        ...
        reject = math.floor(lstm_prediction_lst_length / math.e)
        ...
        for i in range(reject):
            ...
            x = self.dictionary[candidate_lst[i]]
            y, v = self(x, v)
            y = y.squeeze()
            w = y.argmax()
            score = self.candidate_score[w]
        ...
        for j in range(len(remaining_candidates)):
            ...
            if remaining_candidates[j] >= max:
                ...
                x = self.dictionary[candidate_lst[j]]
                y, v = self(x, v)
                y = y.squeeze()
                w = y.argmax()
                score = self.candidate_score[w]
...
def self_train():
    ...
    net = Net20211217()
    net.set(dictionary, candidate_score, 3)
    opt = tr.optim.SGD(net.parameters(), lr=0.01)

    for epoch in range(2000): #epoch in range(2000):
    ...
    for i in range(500):  # run 500 experiments
        ...
        print(net.make_decision(new_candidates))
    ...
    # with open('net20211217.pkl', 'wb') as handle:
    #     pickle.dump(net, handle)  # like_people is a list with data

    tr.save(net,'net20211217.torch')
    #the_model = tr.load('net20211217.torch')
